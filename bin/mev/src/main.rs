//! # MEV
//!
//! MEV is a testnet OP Stack rollup aimed at enabling experimentation of bleeding edge
...
use mev_node::{
    ...
    chainspec::MEVChainSpecParser,
    forwarder::forward_raw_transactions,
    node::MEVNode,
    rpc::{EthApiExt, EthApiOverrideServer},
};
use mev_wallet::{MEVWallet, MEVWalletApiServer, RethUpstream};
use mev_walltime::{MEVWallTime, MEVWallTimeRpcApiServer};
...
fn main() {
    ...
    if let Err(err) =
        Cli::<MEVChainSpecParser, RollupArgs>::parse().run(|builder, rollup_args| async move {
            let wallet = sponsor()?;
            let address = wallet
                .as_ref()
            ...
            let handle = builder
                .with_types_and_provider::<MEVNode, BlockchainProvider2<_>>()
                .with_components(MEVNode::components(&rollup_args))
                .with_add_ons(
                ...
                .extend_rpc_modules(move |ctx| {
                    ...
                    let walltime = MEVWallTime::spawn(ctx.provider().canonical_state_stream());
