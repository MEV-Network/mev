...
/// The mev walltime endpoint.
#[derive(Debug, Clone)]
pub struct MEVWallTime {
    inner: Arc<MEVWallTimeInner>,
}
...
/// Implementation of the MEV `mev_getWallTimeData` endpoint.
#[derive(Debug, Default)]
struct MEVWallTimeInner {
...
/// Rpc endpoints
#[cfg_attr(not(test), rpc(server, namespace = "mev"))]
#[cfg_attr(test, rpc(server, client, namespace = "mev"))]
pub trait MEVWallTimeRpcApi {
...
#[async_trait]
impl MEVWallTimeRpcApiServer for MEVWallTime {
    async fn get_timedata(&self) -> RpcResult<WallTimeData> {
