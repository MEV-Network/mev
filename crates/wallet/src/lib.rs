//! # MEV wallet.
//!
//! Implementations of a custom `wallet_` namespace for MEV experiment 1.
//!
//! - `mev_sendTransaction` that can perform service-sponsored [EIP-7702][eip-7702] delegations
//!   and send other service-sponsored transactions on behalf of EOAs with delegated code.
//!
//! # Restrictions
//!
//! `mev_sendTransaction` has additional verifications in place to prevent some
...
pub trait Upstream {
    ...
    async fn get_code(&self, address: Address) -> Result<Bytes, MEVWalletError>;

    /// Estimate the transaction request's gas usage and fees.
    async fn estimate(
        &self,
        tx: &TransactionRequest,
    ) -> Result<(u64, Eip1559Estimation), MEVWalletError>;

    /// Sign the transaction request and send it to the upstream.
    async fn sign_and_send(&self, tx: TransactionRequest) -> Result<TxHash, MEVWalletError>;
}
    ...
    T: Transport + Clone,
{
    ...
    async fn get_code(&self, address: Address) -> Result<Bytes, MEVWalletError> {
        self.provider
            .get_code_at(address)
            .await
            .map_err(|err| MEVWalletError::InternalError(err.into()))
    }
    ...
    async fn estimate(
        ...
        Ok((
            estimate.map_err(|err| MEVWalletError::InternalError(err.into()))?,
            fee_estimate.map_err(|err| MEVWalletError::InternalError(err.into()))?,
        ))
    }
    ...
    Eth: FullEthApi + Send + Sync,
{
    ...
    async fn get_code(&self, address: Address) -> Result<Bytes, MEVWalletError> {
        let state =
            self.provider.latest().map_err(|err| MEVWalletError::InternalError(err.into()))?;
    ...
    async fn estimate(
        ...
        Ok((
            estimate
                ...
                .map_err(|err| MEVWalletError::InternalError(eyre::Report::new(err)))?,
            fee_estimate
...
impl From<MEVWalletError> for jsonrpsee::types::error::ErrorObject<'static> {
    fn from(error: MEVWalletError) -> Self {
...
/// Implementation of the MEV `wallet_` namespace.
#[derive(Debug)]
pub struct MEVWallet<T> {
    inner: Arc<MEVWalletInner<T>>,
}

impl<T> MEVWallet<T> {
    /// Create a new MEV wallet module.
    pub fn new(upstream: T, chain_id: ChainId) -> Self {
        let inner = MEVWalletInner {
            upstream,
            chain_id,
    ...
    T: Upstream + Sync + Send + 'static,
{
    async fn send_transaction(&self, mut request: TransactionRequest) -> RpcResult<TxHash> {
            ...
            (false, Some(TxKind::Call(addr))) => {
                ...
                match code.as_ref() {
                    ...
                    [0xef, 0x01, 0x00, address @ ..] => {
                        ...
                        if addr.is_zero() {
                            self.inner.metrics.invalid_send_transaction_calls.increment(1);
                            return Err(MEVWalletError::IllegalDestination.into());
                        }
                    }
        ...
        if estimate >= 350_000 {
            self.inner.metrics.invalid_send_transaction_calls.increment(1);
            return Err(MEVWalletError::GasEstimateTooHigh { estimate }.into());
        }
...
fn validate_tx_request(request: &TransactionRequest) -> Result<(), MEVWalletError> {
...
struct WalletMetrics {
    /// Number of invalid calls to `mev_sendTransaction`
    invalid_send_transaction_calls: Counter,
    /// Number of valid calls to `mev_sendTransaction`
    valid_send_transaction_calls: Counter,
}

#[cfg(test)]
mod tests {
    use crate::{validate_tx_request, MEVWalletError};
    ...
    fn no_value_allowed() {
        assert!(matches!(
            ...
            Err(MEVWalletError::ValueNotZero)
        ));
