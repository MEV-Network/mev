...
// This is new endpoint `mev_sendTransaction`, upper test will be deprecate in the future.
#[tokio::test]
async fn test_new_wallet_api() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tx_hash: B256 = provider.client().request("mev_sendTransaction", vec![tx]).await?;
...
async fn test_withdrawal_proof_with_fallback() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let withdrawal_contract_response: EIP1186AccountProofResponse = provider
        ...
        .request(
            "eth_getProof",
            (mev_common::WITHDRAWAL_CONTRACT, vec![B256::ZERO], block_number),
        )
        .await?;
