...
DOCKER_IMAGE_NAME ?= ghcr.io/MEV-Network/mev
...
build-%:
	RUSTFLAGS="-C link-arg=-lgcc -Clink-arg=-static-libgcc" \
		cross build --bin mev --target $* --features "$(FEATURES)" --profile "$(PROFILE)"
...
define docker_build_push
	...
	cp $(BUILD_PATH)/x86_64-unknown-linux-gnu/$(PROFILE)/mev $(BIN_DIR)/amd64/mev

	$(MAKE) build-aarch64-unknown-linux-gnu
	mkdir -p $(BIN_DIR)/arm64
	cp $(BUILD_PATH)/aarch64-unknown-linux-gnu/$(PROFILE)/mev $(BIN_DIR)/arm64/mev
